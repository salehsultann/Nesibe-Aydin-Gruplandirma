...
function enableControls() {
    ...
    // Hide spinner
    controls.recomputeButton.innerHTML = 'Çalıştır'
}
...
function playerName(i) {
    return playerNames[i] ? playerNames[i] : `Öğrenci ${i+1}`
}
...
function downloadCsv() {
    ...
    link.setAttribute("download", "nesibe_aydin.csv")
...
function renderResults() {
    ...
    if (lastResults) {
        lastResults.rounds.forEach((round, roundIndex) => {
            ...
            header.textContent = `Eşleşme ${roundIndex+1}`
            const conflictScore = document.createElement('div')
            conflictScore.classList.add('conflictScore')
            conflictScore.textContent = `Hata Sayısı: ${lastResults.roundScores[roundIndex]}`
            ...
            round.forEach((group, groupIndex) => {
                ...
                groupName.textContent = `Takım ${groupIndex + 1}`
        ...
        if (lastResults.done) {
            ...
            if (startTime) {
                ...
                elapsedTime.textContent = `${elapsedSecs} saniyede hesaplandı.`
            } else {
                elapsedTime.textContent = `Datadan geri yüklendi.`
            }
            ...
            resultsDiv.appendChild(document.createTextNode('Hesaplanıyor...'));
        }
    }
}
